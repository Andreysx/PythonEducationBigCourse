...
def quicksort_2(array: list):
    if len(array) < 2:  # Базовый случай массивы с 0 и 1 элементами уже отсортированы
        ...
        less = [i for i in array if i < pivot]  # Подмассив элементов меньших опорного(применяется List comprehension)
        greater = [i for i in array if i > pivot]  # Подмассив элементов больших опорного(применяется List comprehension)
        return quicksort(less) + [pivot] + quicksort(greater)
